...
public class SysRoleController {
	...
	public Result<List<SysRole>> queryall() {
		...
		//是否开启系统管理模块的多租户数据隔离【SAAS多租户模式】
		if(MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL && tenantId != -1){
			query.eq(SysRole::getTenantId, tenantId);
		}
		//------------------------------------------------------------------------------------------------
	...
	public Result<List<SysRole>> queryallNoByTenant(SysRole sysRole) {
		Result<List<SysRole>> result = new Result<>();
		LambdaQueryWrapper<SysRole> query = new LambdaQueryWrapper<SysRole>();
		if (sysRole.getTenantId() != 0) {
			query.eq(SysRole::getTenantId, sysRole.getTenantId());
		}
		List<SysRole> list = sysRoleService.list(query);
		if(list==null||list.size()<=0) {
			result.error500("未找到角色信息");
