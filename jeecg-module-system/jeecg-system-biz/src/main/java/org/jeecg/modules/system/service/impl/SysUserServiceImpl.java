...
/*	@Override
	...
	public void updateUserDepart(String username,String orgCode,Long loginTenantId) {
...
//		this.save(user);  //保存角色的时候已经添加过一次了
	...
	public void saveUser(SysUser user, String selectedRoles, String selectedDeparts, String relTenantIds) {
		...
		Set<Long> relTenantIdList = new HashSet<>();
		//step.2 保存角色
		if(oConvertUtils.isNotEmpty(selectedRoles)) {
			String[] arr = selectedRoles.split(",");
			for (String roleId : arr) {
				SysRole sysRole = sysRoleMapper.selectById(roleId);
				relTenantIdList.add(sysRole.getTenantId());
				SysUserRole userRole = new SysUserRole(user.getId(), roleId);
				sysUserRoleMapper.insert(userRole);
			}
		}
		...
		this.saveUserTenant(user.getId(), relTenantIdList);
	...
	public void editUser(SysUser user, String roles, String departs, String relTenantIds) {
		...
		if(oConvertUtils.isNotEmpty(roles)) {
			...
			for (String roleId : arr) {
				...
				SysRole sysRole = sysRoleMapper.selectById(roleId);
				tenantIds.add(sysRole.getTenantId());
			}
		}

		//根据角色对应的租户 更新用户租户信息
		this.editUserTenants(user.getId(), tenantIds);
    ...
    @Override
    public List<SysUser> getQuitList(Long tenantId) {
	...
	public Result<JSONObject>  setLoginTenant(SysUser sysUser, JSONObject obj, String username, Result<JSONObject> result){
        ...
        List<Long> tenantIdList = relationMapper.getTenantIdsNoStatus(sysUser.getId());
    ...
    private void setUserTenantIds(SysUser sysUser) {
		if(ObjectUtils.isNotEmpty(sysUser)) {
			List<Long> list  = relationMapper.getTenantIdsNoStatus(sysUser.getId());
			if(null!=list && list.size()>0){
     ...
     * @param relTenantIds
     */
    private void saveUserTenant(String userId, Set<Long> relTenantIds) {
        if (CollectionUtil.isNotEmpty(relTenantIds)) {
            for (Long tenantId : relTenantIds) {
			...
			if (MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL) {
				...
				if(oConvertUtils.isNotEmpty(tenantId)){
					...
					if(count == 0){
						SysUserTenant relation = new SysUserTenant();
						relation.setUserId(userId);
						relation.setTenantId(Long.valueOf(tenantId));
						relation.setStatus(CommonConstant.STATUS_1);
						relationMapper.insert(relation);
					}
				}
     ...
     * @param relTenantIds
     */
    private void editUserTenants(String userId, Set<Long> relTenantIds) {
        LambdaQueryWrapper<SysUserTenant> query = new LambdaQueryWrapper<>();
        query.eq(SysUserTenant::getUserId, userId);
        //数据库的租户id
		List<Long> oldTenantIds = relationMapper.getTenantIdsNoStatus(userId);
        ...
        if (oConvertUtils.isEmpty(relTenantIds) && CollectionUtils.isNotEmpty(oldTenantIds)) {
			...
			if(oConvertUtils.isNotEmpty(relTenantIds) && CollectionUtils.isNotEmpty(oldTenantIds)){
				...
				List<Long> deleteTenantIdList = oldTenantIds.stream().filter(item -> !relTenantIds.contains(item)).collect(Collectors.toList());
				for (Long tenantId : deleteTenantIdList) {
					this.deleteTenantByUserId(userId, tenantId);
				}
				//找到原来租户的用户id与新的租户id不同之处，进行新增
				Set<Long> tenantIds = relTenantIds.stream().filter(item -> !oldTenantIds.contains(item)).collect(Collectors.toSet());
     ...
     * @param userId
     */
    private void deleteTenantByUserId(String userId,Long tenantId){
        LambdaQueryWrapper<SysUserTenant> query = new LambdaQueryWrapper<>();
        query.eq(SysUserTenant::getUserId, userId);
        if(oConvertUtils.isNotEmpty(tenantId)){
            query.eq(SysUserTenant::getTenantId, tenantId);
	...
	public void batchEditUsers(JSONObject json) {
		...
		if(oConvertUtils.isNotEmpty(selecteddeparts)) {
			//查询当前租户的部门列表
			Integer currentTenantId = oConvertUtils.getInt(TenantContext.getTenant(), -1);
	...
	public DepartAndUserInfo searchByKeyword(String keyword) {
		...
		if(oConvertUtils.isNotEmpty(keyword)){
			...
			String str = oConvertUtils.getString(TenantContext.getTenant(), "-1");
			Long tenantId = Long.valueOf(str);
			if(MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL){
