...
public class SysUserDepartServiceImpl extends ServiceImpl<SysUserDepartMapper, SysUserDepart> implements ISysUserDepartService {
	...
	public IPage<SysUser> queryDepartUserPageList(String departId, String username, String realname, int pageSize, int pageNo,String id) {
		...
		if(oConvertUtils.isEmpty(departId)){
			...
			if(MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL && tenantId != -1){
				...
				List<String> userIdList = userTenantMapper.getUserIdsByTenantId(tenantId);
    ...
    @Override
    public IPage<SysUser> getUserInformation(Long tenantId, String departId, String keyword, Integer pageSize, Integer pageNo) {
        IPage<SysUser> pageList = null;
        // 部门ID不存在 直接查询用户表即可
	...
	@Override
	public IPage<SysUser> getUserInformation(Long tenantId, String departId,String roleId, String keyword, Integer pageSize, Integer pageNo) {
		IPage<SysUser> pageList = null;
		// 部门ID不存在 直接查询用户表即可
		Page<SysUser> page = new Page<>(pageNo, pageSize);
		LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
