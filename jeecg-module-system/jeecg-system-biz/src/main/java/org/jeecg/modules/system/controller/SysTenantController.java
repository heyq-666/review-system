...
public class SysTenantController {
     ...
     * @return
     */
    @GetMapping(value = "/allPackList")
    //@RequiresPermissions("system:tenant:allPackList")
    public Result<List<SysTenantPack>> queryAllPackPageList(SysTenantPack sysTenantPack) {

        Result<List<SysTenantPack>> result = new Result<List<SysTenantPack>>();

        LambdaQueryWrapper<SysTenantPack> query = new LambdaQueryWrapper<>();
        query.eq(SysTenantPack::getStatus, 1);
        if (StrUtil.isNotBlank(sysTenantPack.getPackName())) {
            query.like(SysTenantPack::getPackName, sysTenantPack.getPackName());
        }

        //此处查询忽略时间条件
        List<SysTenantPack> ls = sysTenantPackService.list(query);
    ...
    public Result<Long> saveTenantJoinUser(@RequestBody SysTenant sysTenant){
        ...
        //------------------------------------------------------------------------------------------------
        Long tenantId = sysTenantService.saveTenantJoinUser(sysTenant, sysUser.getId());
        result.setSuccess(true);
    ...
    public Result<Long> joinTenantByHouseNumber(@RequestBody SysTenant sysTenant){
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        Long tenantId = sysTenantService.joinTenantByHouseNumber(sysTenant, sysUser.getId());
        Result<Long> result = new Result<>();
        if(tenantId != 0){
    ...
    public Result<IPage<SysUserTenantVo>> getUserTenantPageList(@RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        ...
        IPage<SysUserTenantVo> list = relationService.getUserTenantPageList(page, Arrays.asList(userTenantStatus.split(SymbolConstant.COMMA)), user, Long.valueOf(tenantId));
    ...
    public Result<List<TenantPackUserCount>> loadAdminPackCount(@RequestParam("tenantId") Long tenantId){
    ...
    public Result<?> getTenantPackApplyUsers(@RequestParam("tenantId") Long tenantId){
    ...
    public Result<TenantDepartAuthInfo> queryTenantAuthInfo(@RequestParam(name="id",required=true) String id) {
        TenantDepartAuthInfo info = sysTenantService.getTenantDepartAuthInfo(Long.valueOf(id));
    ...
    public Result<?> generateAppletsQrCode(SysTenant sysTenant) {
        Result result = new Result();
        if (sysTenant.getId() == null){
            result.setSuccess(false);
            result.setCode(300);
            result.setMessage("租户ID为空，无法生成二维码");
            return result;
        }
        SysTenant sysTenantDb = sysTenantService.getById(sysTenant.getId());
        if (sysTenantDb == null) {
            result.setSuccess(false);
            result.setCode(301);
            result.setMessage("租户不存在，无法生成二维码");
            return result;
        }
        //生成二维码
        ...
        sysTenantService.saveOrUpdate(sysTenantDb);
