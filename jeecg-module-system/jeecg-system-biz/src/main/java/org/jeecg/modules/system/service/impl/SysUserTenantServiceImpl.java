...
public class SysUserTenantServiceImpl extends ServiceImpl<SysUserTenantMapper, SysUserTenant> implements ISysUserTenantService {
    ...
    @Override
    public List<String> getUserIdsByTenantId(Long tenantId) {
    ...
    @Override
    public IPage<SysUserTenantVo> getUserTenantPageList(Page<SysUserTenantVo> page, List<String> status, SysUser user, Long tenantId) {
        ...
        if (userIds != null && userIds.size() > 0) {
            ...
            tenantPageList.forEach(item -> {
                ...
                //查询用户的租户ids
                List<Long> list = userTenantMapper.getTenantIdsNoStatus(item.getId());
                if (oConvertUtils.isNotEmpty(list)) {
    ...
    public void putCancelQuit(List<String> userIds, Long tenantId) {
        userTenantMapper.putCancelQuit(userIds, tenantId);
    }

    @Override
    public Integer userTenantIzExist(String userId, Long tenantId) {
