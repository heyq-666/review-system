...
//	}
    ...
    public Result<List<SysUser>> getQuitList(HttpServletRequest req) {
        Long tenantId = oConvertUtils.getLong(TokenUtils.getTenantIdByRequest(req),0);
    ...
    public Result<String> putCancelQuit(@RequestBody JSONObject jsonObject, HttpServletRequest request){
        ...
        Long tenantId = oConvertUtils.getLong(TokenUtils.getTenantIdByRequest(request),0);
        //将状态改成未删除
        if (StringUtils.isNotBlank(userIds)) {
